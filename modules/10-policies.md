...
Policies are where regulation meets reality. They're also the most common **Major NC** in ISO audits ("policy not updated," "policy not approved," "no evidence of training on policy"). A small company can have world-class tooling and still fail an audit because its policy set is a mess.
...
|-------|-----------|----------|----------------|
| **Policy** | What & why (mandatory) | All employees | Annually |
...
- **Guideline:** "Consider passwordless (passkeys) for all production systems."
- **Record:** the ticket, approval, PIM log — evidence the procedure was executed.

Mixing these is the single largest source of audit confusion.
...
1. **Document metadata:** title, version, owner, approver, effective date, next review date, distribution list.
2. **Purpose** — one paragraph.
3. **Scope** — people, systems, locations.
4. **Definitions** — only as needed.
5. **Policy statements** — use **MUST / SHOULD / MAY** precisely.
6. **Roles & responsibilities** — RACI if helpful.
7. **Exceptions** — who can grant, process to request, time bounds.
...
Keep it **short** (3–5 pages). Long policies aren't read; short policies are.
...
You'll typically need ~20 policies. Group into themes:
...
### Emerging
- AI Acceptable Use Policy (added in 2023+)
- BYOD / Mobile Device Policy

If you have these 20, versioned, approved, trained, and reviewed — you've eliminated 30% of typical audit findings.
...
1. **Author** — draft.
2. **Review** — legal, security, privacy, impacted business owners.
...
- Employee acknowledgement — you must be able to prove **each employee saw the policy and accepted it**.

---

## 10.5 Exception management — the quiet killer

Every policy needs an **exception process**. Without it, teams silently ignore the policy.

Exception request should capture:
- What's being excepted; which policy clause.
- Why (business justification).
- Risk introduced and compensating controls.
- Approver (risk owner + security + privacy as relevant).
- Expiry date (time-bound, usually ≤12 months).
- Review conditions.

Track all exceptions in a register. Expired exceptions either get renewed with justification or are closed (control returns).
...
- **On hire** — baseline security + privacy training + role-specific (devs get SDLC, admins get privileged access).
- **Annually** — refresher.
...
Free templates are a fine starting point; they become harmful if dropped in unchanged.

- **Customise scope, roles, and statements** to your org.
- **Remove sections that don't apply** (don't list HSMs if you have none).
- **Align numbers** with reality (don't write "backups tested quarterly" if you do it annually).
- **Language consistency** — British vs American English, defined terms capitalised.
- **Review with a lawyer** before publishing anything external-facing (Privacy Notice, ToS).

SANS templates are excellent starting points; plan to rewrite 50%.

---

## 10.8 Common beginner mistakes

- Policies written by security team alone, disconnected from how work happens. Co-author with the team that will comply.
- Over-long policies nobody reads. Put standards/procedures in separate docs.
- Version numbers that never change, despite edits. Use a real document-control system.
- No approval record. An unsigned policy is, for audit purposes, not a policy.
- Policies contradicting each other. Review annually for consistency.

## 10.9 Mini-exercise (30 min)

1. Open a [SANS Acceptable Use Policy template](https://www.sans.org/information-security-policy/).
2. Rewrite it for a 50-person EdTech startup — scope, roles, 8–12 policy statements using MUST/SHOULD, exception process, enforcement.
